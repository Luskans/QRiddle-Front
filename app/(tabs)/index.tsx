...
import TopRiddleLeaderboard from '@/components/(riddles)/TopRiddleLeaderboard';


export default function HomeScreen() {
  const { isDark } = useThemeStore();
  const {  } = useRiddleStore();
  const {  } = useGameSessionStore();
  const {  } = useLeaderboardStore();

  useFocusEffect(
    useCallback(() => {
      // TODO : fetch game-sessions count, game-sessions active, riddle count
    }, [])
  );
      ...
      <View className='py-20 gap-12'>
        ...
        <View className='flex gap-2 px-6'>
          ...
          {/* <Text className='dark:text-light text-lg'>
            ...
            {notificationsCount > 1 ? 'notifications' : 'notification'} en attente.
          </Text> */}
        ...
        {/* <View className='flex-col gap-10 px-6'> */}
          ...
          {/* <Link href="/users/me/game-sessions" asChild>
            <TouchableOpacity className='flex-row items-center gap-3'>
          ...
          {/* RIDDLES CREATED */}
          {/* <Link href="/users/me/riddles/created" asChild>
            <TouchableOpacity className='flex-row items-center gap-3'>
              <Ionicons name="footsteps-outline" size={22} color={isDark ? colors.light : colors.dark} />
        ...
        {/* LEADERBOARD */}
        <View className='flex gap-2 px-6'>
          <Link href="/leaderboards/global" asChild className='mb-6'>
            <TouchableOpacity className='flex-row items-center gap-3'>
          ...
          <TopRiddleLeaderboard riddleId={'1'} />
        </View>
